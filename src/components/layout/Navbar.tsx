...
const Navbar = ({ personalInfo }: { personalInfo: PersonalInfo | null }) => {
  ...
  // Hydration guard + localStorage + system preference detection
  useEffect(() => {
    const saved = localStorage.getItem('theme');
    const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
    const newDark = saved === 'dark' || (!saved && prefersDark);
    setDark(newDark);
        ...
        <div className="container mx-auto px-4 h-12">
          <div className="flex items-center justify-around h-full">
            ...
            <Button
              ...
              size="icon"
              onClick={toggleTheme}
              className="flex flex-col items-center justify-center gap-0.5 text-sm transition-all duration-300 text-muted-foreground hover:text-foreground h-9 w-9"
              aria-label="Toggle dark mode"
            >
              {mounted ? (dark ? sunIcon : moonIcon) : moonIcon}
            </Button>
