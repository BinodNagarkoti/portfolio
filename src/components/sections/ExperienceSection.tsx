...
const ExperienceSection = async () => {
  ...
  return (
    <SectionWrapper id="experience" title="Work Experience" subtitle="My Career Journey">
          ...
          <div key={exp.id} className="relative pl-10 md:pl-0">
            ...
            <div className="md:flex md:items-center md:justify-between">
              ...
              <div className={`md:w-[calc(50%-2rem)] ${index % 2 === 0 ? 'md:order-1' : 'md:order-2'}`}>
                <Card className="bg-card/50 backdrop-blur-xs shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                  <CardHeader>
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                        <div>
                            <CardTitle className="text-lg font-bold font-headline">{exp.job_title}</CardTitle>
                            <CardDescription className='mt-1'>{exp.company} &bull; {exp.location}</CardDescription>
                        </div>
                        <div className="text-xs text-muted-foreground whitespace-nowrap mt-1 sm:mt-0">{formatDateRange(exp.start_date!, exp.end_date)}</div>
                    </div>
                  </CardHeader>
                  <CardContent>
               ...
               <div className={`hidden md:block md:w-[calc(50%-2rem)] ${index % 2 === 0 ? 'md:order-2' : 'md:order-1'}`}></div>
            </div>
          </div>
        ))}
      </div>
